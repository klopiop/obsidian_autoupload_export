...
- `autoupload——export/`：提供含图片上传的导出功能，带 TypeScript 源码。
- `obsidian-image-auto-upload-plugin/`：自动上传图片（仅打包产物）。
- `obsidian-enhancing-export/`：增强导出并支持 Pandoc/Lua 过滤（仅打包产物）。

## 快速开始（autoupload——export）
1. 将目录复制到 Obsidian 插件目录：`<vault>/.obsidian/plugins/autoupload——export`。
...
## 代码风格（autoupload——export/main.ts）
...
## 文件与版本管理
- 源码：仅 `autoupload——export/main.ts` 可编辑；其他插件的 `main.js` 为打包产物，修改需重新构建源。
...
## 忽略文件建议
见 `autoupload——export/.gitignore`，主要忽略 `node_modules/`、构建产物（除 `main.js`）、锁文件及 OS 垃圾文件。
